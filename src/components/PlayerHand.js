    ...
    return (
      <ImageBackground
        ...
        {this.props.gameStart ? (
          <View style={styles.topArea}>
            {this.props.playerHand.map((card, i) => {
              return (
                <Animated.View
                  key={i}
                  style={this.getCardWrapperStyle(i,this.props.playerHand.length)}>
                  <TouchableOpacity>
                    <Image resizeMode="stretch" source={card.image} style={[styles.card]}/>
                  </TouchableOpacity>
                </Animated.View>
              );
            })}
          ...
          <View style={styles.topArea}>
            <Animatable.Text style={{ fontSize: 24, color: 'white', textAlign: 'center',}}
              animation="pulse"
              easing="ease-out"
              iterationCount="infinite">
              {this.props.gameStarting ? 'Dealing Cards...' : 'Please Tap the Middle Deck to Start the Game'}
            </Animatable.Text>
          </View>
        )}
      </ImageBackground>
...
function mapStateToProps(state) {
  return {
    ...
    playerHand: state.GameReducer.playerHand,
